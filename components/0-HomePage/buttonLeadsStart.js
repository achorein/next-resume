...
class buttonLeadsStart extends Component {
            ...
            // },
                ...
                preConfirm: () => {
                    ...
                    if(erreurEmail === false) {
                        ...
                        var objectEmail = { email : document.getElementById('email').value, free : false}
    
                       fetch(`https://ns3296606.ip-5-135-152.eu:8443/api/user/prospect/email?email=`+objectEmail.email)
                        .then(response => {
    ...
    render() {
        ...
        return (
            <a href="#/" className={`${this.props.buttonStyle}`} onClick={this.openModal}>{this.props.buttonTitle}</a>
        )
    }
}
